...
export default async function McpPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const project = await prisma.project.findUniqueOrThrow({ where: { id: projectId }, include: { vault: true } });
  const resources = (await listMcpResources("PI")).filter((resource) => resource.uri.includes(`/project/${project.id}/`));
  return (
